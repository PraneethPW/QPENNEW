...
from seq_utils import normalize_tagging_schema, ot2bio, ot2bioes
...
def get_tag_list(task, tagging_schema):
    """ Set up the tag list """
    task = task.lower()
    tagging_schema = normalize_tagging_schema(tagging_schema).lower()
    if task == 'absa':
    ...
    with open(file_path, 'r', encoding='UTF-8') as fp:
        ...
        for line in fp:
            ...
            if line != '':
                ...
                normalized_schema = normalize_tagging_schema(tagging_schema).lower()
                if normalized_schema == 'bioes':
                    tags = ot2bioes(tags, task)
                elif normalized_schema == 'bio':
...
def detect_available_languages(data_dir, split='train'):
    top_data_dir = os.path.join(data_dir, 'rest')
    languages = set()
    if not os.path.exists(top_data_dir):
        return []
    for file_name in os.listdir(top_data_dir):
        if file_name.startswith('gold-') and file_name.endswith(f'-{split}.txt'):
            lang = file_name[len('gold-'):-len(f'-{split}.txt')]
            languages.add(lang)
    return sorted(languages)


def resolve_multilingual_languages(args):
    if getattr(args, 'multilingual_langs', ''):
        return [lang.strip() for lang in args.multilingual_langs.split(',') if lang.strip()]
    langs = detect_available_languages(args.data_dir, split='train')
    return [lang for lang in langs if lang != args.src_lang]


def get_mode_file_names(args, mode):
    exp_type = args.exp_type

    if mode == 'dev':
        return f'gold-{args.src_lang}-dev.txt'
    if mode == 'test':
        return f"gold-{args.tgt_lang}-test.txt"
    if mode == 'unlabeled':
        return f"gold-{args.tgt_lang}-train.txt"
    if mode == 'unlabeled_mtl':
        return [f"gold-{l}-train.txt" for l in resolve_multilingual_languages(args)]
    if mode == 'train':
        if exp_type == 'supervised':
            assert args.src_lang == args.tgt_lang, "Src and Tgt langs should be the same under supervised setting!"
            return f"gold-{args.src_lang}-train.txt"
        if exp_type == 'smt':
            return f"{exp_type}-{args.tgt_lang}-train.txt"
        if exp_type.startswith('mtl'):
            languages = resolve_multilingual_languages(args)
            file_name_or_list = ['gold-en-train.txt']
            file_name_or_list += [f'smt-{l}-train.txt' for l in languages]
        ...
        if exp_type == 'acs_mtl':
            ...
            file_name_or_list = ["gold-en-train.txt"]
            file_name_or_list += [f'smt-{l}-train.txt' for l in languages]
            file_name_or_list += [f'cs_en-{l}-train.txt' for l in languages]
            file_name_or_list += [f'cs_{l}-en-train.txt' for l in languages]
            return file_name_or_list
    raise Exception("Invalid mode `{mode}`, should be one of the [train, dev, test]")


def load_raw_examples(args, mode):
    file_name_or_list = get_mode_file_names(args, mode)
    top_data_dir = f"{args.data_dir}/rest"
    if isinstance(file_name_or_list, str):
        file_path = f"{top_data_dir}/{file_name_or_list}"
        return read_examples_from_file(file_path, args.task, args.tagging_schema, ratio=args.data_select)
    file_path = [f"{top_data_dir}/{f}" for f in file_name_or_list]
    return read_examples_from_multiple_file(file_path, args.task, args.tagging_schema, args.exp_type, ratio=0.5)


def write_tagged_predictions(file_path, texts, gold_tags, pred_tags):
    with open(file_path, 'w', encoding='UTF-8') as fp:
        for sent_idx, words in enumerate(texts):
            for token, gold_tag, pred_tag in zip(words, gold_tags[sent_idx], pred_tags[sent_idx]):
                fp.write(f"{token}\t{gold_tag}\t{pred_tag}\n")
            fp.write("\n")
...
def build_or_load_dataset(args, tokenizer, mode='train'):
    ...
    if os.path.exists(cached_features_file) and not args.ignore_cached_data:
        ...
        encodings = tokenizer(texts, is_split_into_words=True, return_offsets_mapping=True, truncation=True, padding='max_length', max_length=args.max_seq_length)
