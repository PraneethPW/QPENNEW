...
class PositionEmbedding(torch.nn.Module):
    def __init__(self, embed_dim, input_dim=1, zero_phase=False, device=None):
    ...
    def forward(self, x):
        ...
        positions = torch.arange(time_stamps, device=x.device).unsqueeze(-1)
...
class QMeasurement(torch.nn.Module):
    def __init__(self, embed_dim, device=None):
        ...
        self.kernel = torch.nn.Parameter(
            torch.stack([torch.eye(embed_dim), torch.zeros(embed_dim, embed_dim)], dim=-1)
        )
...
class QMixture(torch.nn.Module):
    ...
    def __init__(self, use_weights=True, device=None):
