...
from seq_utils import compute_metrics_absa, convert_tags_to_bioes, normalize_tagging_schema
from data_utils import (
    build_or_load_dataset,
    get_tag_vocab,
    load_raw_examples,
    write_results_to_log,
    write_tagged_predictions,
)
...
def init_args():
    ...
    # basic settings
    parser.add_argument("--tfm_type", default='', type=str, required=True,
    ...
    parser.add_argument("--tgt_lang", default='nl', type=str, required=True, help="target language, sp = es, du = nl")

    parser.add_argument("--data_select", default=1.0, type=float,
                        help="ratio of the selected data to train, 1 is to use all data")
    parser.add_argument('--tagging_schema', type=str, default='BIOES')
    parser.add_argument('--label_path', type=str, default='')
    parser.add_argument('--multilingual_langs', type=str, default='',
                        help="Comma-separated target languages for multilingual training. "
                             "If empty, languages are auto-detected from data/rest.")
    parser.add_argument('--prediction_output_format', type=str, default='BIOES',
                        help="Token-level export format for tagged outputs.")
    ...
    parser.add_argument("--trained_teacher_paths", type=str, default='./outputs/base/checkpoint', help="path of the trained model")
    parser.add_argument("--do_eval", action='store_true', default=True, help="Whether to run eval on the dev/test set.")
    ...
    parser.add_argument("--adam_epsilon", default=2.8e-5, type=float, help="Epsilon for Adam optimizer.")
    ...
    parser.add_argument("--train_begin_saving_step", type=int, default=1500, help="Starting point of evaluation.")
...
def train(args, train_dataset, model, tokenizer):
    ...
    if args.local_rank in [-1, 0]:
       try:
           ...
           logger.warning("TensorBoard writer is unavailable, continuing without it: %s", exc)
           tb_writer = NullSummaryWriter()
    ...
    logger.info("  Learning rate = %f", args.adam_epsilon)
...
def evaluate(args, eval_dataset, model, idx2tag, mode, step=None):
    ...
    if mode == 'test':
        ...
        raw_texts, raw_gold_tags = load_raw_examples(args, mode='test')
        exported_gold_tags, exported_pred_tags = [], []
        for sample_idx in range(len(pred_labels)):
            eval_positions = np.where(gold_labels[sample_idx] != -100)[0]
            sample_pred = pred_labels[sample_idx][eval_positions]
            sample_gold = gold_labels[sample_idx][eval_positions]
            pred_tags = [idx2tag[label] for label in sample_pred]
            gold_tags = [idx2tag[label] for label in sample_gold]
            exported_pred_tags.append(convert_tags_to_bioes(pred_tags, args.task, args.tagging_schema))
            exported_gold_tags.append(convert_tags_to_bioes(gold_tags, args.task, args.tagging_schema))
        bioes_file_name = f'{args.saved_model_dir}/{args.src_lang}-{args.tgt_lang}-preds-{step}.bioes.txt'
        write_tagged_predictions(bioes_file_name, raw_texts, exported_gold_tags, exported_pred_tags)
        logger.info(f"Write BIOES predictions to {bioes_file_name}")
    """
...
def main():
    ...
    args = init_args()
    args.tagging_schema = normalize_tagging_schema(args.tagging_schema)
    args.prediction_output_format = normalize_tagging_schema(args.prediction_output_format)
