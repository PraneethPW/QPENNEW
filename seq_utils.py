...
def normalize_tagging_schema(tagging_schema):
    schema = tagging_schema.upper()
    if schema == 'BIEOS':
        return 'BIOES'
    return schema
...
def ot2bioes_ate(ate_tag_sequence):
    return ot2bieos_ate(ate_tag_sequence)
...
def ot2bioes_absa(absa_tag_sequence):
    return ot2bieos_absa(absa_tag_sequence)
...
def ot2bioes(tags, task):
    return ot2bieos(tags, task)
...
def convert_tags_to_bioes(tag_sequence, task, tagging_schema):
    schema = normalize_tagging_schema(tagging_schema)
    if schema == 'OT':
        return ot2bioes(tag_sequence, task)
    if schema == 'BIO':
        if task == 'ate':
            return ot2bieos_ate(bio2ot_ate(tag_sequence))
        if task == 'absa':
            return ot2bieos_absa(bio2ot_absa(tag_sequence))
    if schema == 'BIOES':
        return tag_sequence
    raise Exception("Unsupported tagging schema: {}".format(tagging_schema))
...
def compute_metrics_absa(pred, gold, label_vocab, tagging_schema):
    ...
    for i in range(num_samples):
        ...
        gold_tags = convert_tags_to_bioes(gold_tags, task='absa', tagging_schema=tagging_schema)
        pred_tags = convert_tags_to_bioes(pred_tags, task='absa', tagging_schema=tagging_schema)
...
def compute_metrics_ate(pred, gold, label_vocab, tagging_schema):
    ...
    for i in range(num_samples):
        ...
        pred_tags = [label_vocab[label] for label in pred_labels]
        gold_tags = [label_vocab[label] for label in gold_labels]
        gold_tags = convert_tags_to_bioes(gold_tags, task='ate', tagging_schema=tagging_schema)
        pred_tags = convert_tags_to_bioes(pred_tags, task='ate', tagging_schema=tagging_schema)
